            ...
            try:
                for i, vehicle in enumerate(data['vehicles']):
                    if vehicle['name'] == self.vehicle_name:
                        ...
                        try:
                            self.global_data[j] = VehicleData()
                        except Exception as err:
                            self.logger.error(err)
                            self.global_data.append(VehicleData())
                        self.global_data[j].name = vehicle['name']
                        self.global_data[j].x = vehicle['x']
                        self.global_data[j].y = vehicle['y']
                        self.global_data[j].yaw = vehicle['yaw']
                        self.global_data[j].speed = vehicle['speed']
                        j += 1
